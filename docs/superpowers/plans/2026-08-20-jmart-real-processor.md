...
- **Delivery address comes from the PDF** (`xulydonhang.py:8151-8152`, `Địa chỉ giao hàng:...SĐT nhận hàng:`), unlike Kingfood's hardcoded constant. **CORRECTED post-implementation (final whole-branch review caught this plan text was wrong):** the regex site itself has a soft `if m else None` guard, but real Python does NOT ship a literal `None` into Excel on a miss — `write_to_dondathang_kingfood` (the function both Kingfood and JMart call) applies `delivery = delivery or "KHO SEEDLOG"` (`xulydonhang.py:3865`) before writing ShipTo, so a missing marker in real Python means the order still processes with ShipTo="KHO SEEDLOG". This port deliberately diverges from that: it treats a missing delivery-address match as `ok=false` (clean failure of the whole page) rather than reproducing the `"KHO SEEDLOG"` fallback, since a silently-substituted warehouse address carries no signal anything went wrong. This divergence is undocumented in any golden fixture — the one real sample has a valid address.
